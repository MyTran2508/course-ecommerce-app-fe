...
export const courseProcessApi = createApi({
  ...
  endpoints: (builder) => ({
    getByUserIdAndCourseId: builder.query<DataResponse, {courseId: string, userId: string}>({
    ...
    updateCurrentProgress: builder.mutation<DataResponse, {courseId: string, userId: string}>({
       query: ({courseId, userId}) => {
         return {
